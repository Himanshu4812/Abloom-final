...
export const Header = () => {
  ...
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    const isCurrentlyCollapsed = window.scrollY > 50;
  ...
  React.useEffect(() => {
    const handleScroll = () => {
      const sections = [...links.map((link) => link.href.replace('#', '')), 'plans'];
    ...
    <>
      <header
        className={cn(
          ...
          {
            'opacity-0 -translate-y-full pointer-events-none': !isHeroFinished || (activeSection === 'gallery' && !isGalleryIntro && !open) || (activeSection === 'plans' && !open),
            'bg-gradient-to-r from-[#FDFBF7]/95 via-white/95 to-[#E8F5E9]/95 border-[#1B5E20]/15 backdrop-blur-2xl top-0 md:top-6 md:max-w-5xl md:rounded-full md:border md:shadow-[0_8px_32px_rgba(27,94,32,0.1)] md:left-1/2 md:-translate-x-1/2': isHeroFinished && scrolled && !open,
            'bg-white/95 backdrop-blur-xl top-0': isHeroFinished && open,
            'bg-gradient-to-b from-black/60 to-transparent backdrop-blur-sm top-0': isHeroFinished && !scrolled && !open,
          }
        )}
      >
        <nav
        ...
        >
              ...
              return (
                <a
                  ...
                  className={cn(
                    ...
                    isActive
                      ? (scrolled ? 'text-forest font-bold' : 'text-white font-bold')
                      : (scrolled ? 'text-forest/60 hover:text-forest/90' : 'text-white/70 hover:text-white')
                  )}
                  href={link.href}
                ...
                >
                  ...
                  {isActive && (
                    <span className={cn(
                      ...
                      scrolled ? "bg-gradient-to-r from-primary to-forest" : "bg-white/50"
                    )} />
                  )}
                </a>
              );
          ...
          <button
            ...
            className={cn("md:hidden p-3 transition-colors duration-300", scrolled ? "text-forest" : "text-white")}
      ...
      <div
        className={cn(
          'fixed inset-0 top-[64px] z-[999] bg-white/80 backdrop-blur-lg flex flex-col overflow-hidden transition-all duration-500 md:hidden',
          open
